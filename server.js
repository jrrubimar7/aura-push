...
// CommonJS para evitar fallos de deploy por "import"
...
async function sendNotificationToAll(payloadObj) {
  const payload = JSON.stringify({
    title: payloadObj && payloadObj.title ? payloadObj.title : 'AURA',
    body: payloadObj && payloadObj.body ? payloadObj.body : 'Estoy aquí. ¿Seguimos?',
    url: payloadObj && payloadObj.url ? payloadObj.url : '/'
  });
  ...
  for (const sub of subscriptions) {
    try {
      ...
      console.error('[PUSH ERROR]', e && e.message ? e.message : e);
    }
  }
...
async function maybeSendProactive() {
  ...
  if (result.sent > 0) {
    lastProactiveAt = now;
    pushCount++;
    console.log('[PUSH]', Object.assign({}, result, { pushCount }));
  }
}
...
app.post('/auto', async (req, res) => {
  const result = await sendNotificationToAll({
    title: req.body && req.body.title ? req.body.title : 'AURA',
    body: req.body && req.body.body ? req.body.body : `AURA ∞.Ω · ${pickMessage()}`,
  ...
  res.json(Object.assign({ ok: true, pushCount }, result));
});
...
app.post('/proactive-config', (req, res) => {
  ...
  res.json(Object.assign({ ok: true }, statusPayload()));
});
...
app.listen(PORT, () => {
  console.log('AURA server v5 CJS running on port', PORT);
});
