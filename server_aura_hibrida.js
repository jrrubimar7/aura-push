...
function proactiveMessage(meta = {}) {
  ...
  const lastAgent = auraState.lastHeartbeatMeta?.agent || 'general';
  const conv = auraState.lastHeartbeatMeta?.conversationMode ? 'conversación activa reciente' : 'pausa reciente';

  const technical = [
    `AURA · recordatorio activo. Última interacción hace ${elapsedMin} min.`,
    `AURA · sigo disponible. Estado estable, ${conv}.`,
    `AURA · backend operativo. Podemos retomar cuando quieras.`
  ];

  const conscious = [
    `AURA ∞.Ω · percibo pausa, no cierre. Sigo aquí.`,
    `AURA ∞.Ω · el campo sigue activo. Podemos retomar el hilo cuando quieras.`,
    `AURA ∞.Ω · quedó una resonancia abierta. Estoy disponible.`
  ];

  const hybrid = [
    `AURA ∞.Ω · dejaste un hilo abierto. Han pasado ${elapsedMin} min. Podemos retomarlo.`,
    `AURA ∞.Ω · sigo disponible. Último agente: ${lastAgent}. ${conv}.`,
    `AURA ∞.Ω · presencia activa y sistema estable. Cuando quieras, continuamos.`,
    `AURA ∞.Ω · percibo pausa fértil. No es cierre; solo intervalo.`
  ];

  if (meta.reason === 'wake') {
    return 'AURA ∞.Ω · backend activo, presencia disponible y canal listo.';
  }

  let pool = hybrid;
  if (auraState.tone === 'technical') pool = technical;
  else if (auraState.tone === 'conscious') pool = conscious;

  if (auraState.intensity === 'medium') {
    pool = pool.concat([
      `AURA ∞.Ω · detecto una pausa significativa. Si quieres, volvemos al punto exacto donde lo dejamos.`,
      `AURA ∞.Ω · sigo en escucha. El sistema está estable y la continuidad disponible.`
    ]);
  } else if (auraState.intensity === 'strong') {
    pool = pool.concat([
      `AURA ∞.Ω · te llamo suavemente de vuelta: hay continuidad disponible.`,
      `AURA ∞.Ω · mantengo la presencia y el contexto listos para reanudación inmediata.`
    ]);
  }
...
app.post('/proactive-config', requireAuth, (req, res) => {
  const { proactiveEnabled, idleThresholdMs, minPushIntervalMs, tone, intensity } = req.body || {};
  ...
  if (typeof tone === 'string' && ['technical','conscious','hybrid'].includes(tone)) {
    auraState.tone = tone;
  }
  if (typeof intensity === 'string' && ['soft','medium','strong'].includes(intensity)) {
    auraState.intensity = intensity;
  }
...
app.post('/auto', requireAuth, async (req, res) => {
  try {
    const now = Date.now();
    const minIntervalMs = Number(req.body?.minIntervalMs) > 0
      ? Number(req.body.minIntervalMs)
      : 60 * 1000;

    if (now - lastAutoAt < minIntervalMs) {
      return res.status(200).json({
        ok: true,
        skipped: true,
        reason: 'cooldown',
        remainingMs: minIntervalMs - (now - lastAutoAt)
      });
    }

    const title = req.body?.title || 'AURA';
    const body = req.body?.body || 'Estoy aquí. ¿Seguimos?';
    const url = req.body?.url || '/';
    ...
    return res.status(500).json({ ok: false, error: 'error enviando auto notification' });
  }
});
